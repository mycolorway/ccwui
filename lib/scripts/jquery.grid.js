...
$.widget( "ccw.grid", {
    ...
    _init: function() {
        ...
        if ( opts.data ) {
            ...
            if ( opts.tree ) {
                ...
                opts.localPaging = true;  
                this._pageCount = Math.ceil( opts.data.length / opts.capacity );
    ...
    _generateMarkup: function() {
        ...
        if ( opts.paging && opts.pagingToolbar ) {
    ...
    _pageData: function( data ) {
        ...
        if ( !opts.url || opts.localPaging ) {
            var start = ( this._currentPage - 1 ) * opts.capacity;
            var end = start + opts.capacity;
    ...
    page: function( index ) {
        ...
        if ( !opts.url ) {
            ...
            this.add( this._pageData( opts.data ));
            this._refreshStripe();
            wrapper.height( "auto" );
        } else {  
            $.ajax({
                ...
                success: $.proxy( function( result ) {
                    opts.data = result.data;
                    this._pageCount = Math.ceil(( result.total || opts.data.length ) / opts.capacity );
