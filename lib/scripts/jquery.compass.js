(function ( $ ) {
	...
	function _init ( opts ) {
		...
		setInterval( function(){
			...
			if( ave > opts.pointerA[0] && ave > opts.pointerB[0] ) return;
	...
	function draw( opts, value, ctx ) {
		...
		if ( value * opts.speed[0]  <= opts.pointerA[0] ){
			counter[0] = value * opts.speed[0];
		} else if ( opts.pointerA[0] == 0 ){
			counter[0] = 0;
		} 
		var result = rad ( opts, counter[0] );
		if ( opts.imgA[0] == "id" ){
			...
			ctx.drawImage( document.getElementById( opts.imgA[0] ), - result.dx + opts.imgA[1] , - result.dy + opts.imgA[2] );
		}
		ctx.restore();
		...
		if( opts.pointerB[3] ){
			...
			if ( value * opts.speed[1] <= opts.pointerB[0] ){
				counter[1] = value * opts.speed[1];
			} else if( opts.pointerB[0] == 0 ){
				counter[1] = 0;
			}
			result = rad ( opts, counter[1] );
			if( opts.imgB[0] == "id" ){
				ctx.rotate( result.rad );
				...
				ctx.rotate( result.rad - Math.PI );
				ctx.drawImage( document.getElementById( opts.imgB[0] ), - result.dx + opts.imgB[1] , - result.dy + opts.imgB[2] );
			}
			ctx.restore();
